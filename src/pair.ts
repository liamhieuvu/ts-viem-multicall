...
export function getTotalSupply(pair: Address): Multicall {
  ...
  return {
    ...
    handler: ([resp]): TotalSupplyReturn => (!!resp && resp.status === 'success' ? (resp.result as bigint) : 0n)
  }
}
...
export function getBalances(users: Address[], pair: Address): Multicall {
  ...
  return {
    ...
    handler: (responses): BalancesReturn => responses.map((r) => (r.status === 'success' ? (r.result as bigint) : 0n))
  }
}
